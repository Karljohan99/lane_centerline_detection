...
def train(args):
    ...
    print("Local rank:", local_rank, torch.cuda.device_count())
    #local_rank = 0
    # ==============
    if args.multi_GPU:
        ...
        RNGDetNet.cuda()
        RNGDetNet = torch.nn.parallel.DistributedDataParallel(RNGDetNet, device_ids=[local_rank],output_device=local_rank,find_unused_parameters=True)
    ...
    if args.resume is not None:
        checkpoint = torch.load(f"./{args.savedir}/checkpoints/{args.resume}", map_location='cpu')
        model_without_ddp.load_state_dict(checkpoint['model'])
        opt.load_state_dict(checkpoint['optimizer'])
        sched.load_state_dict(checkpoint['lr_scheduler'])
        start_epoch = checkpoint['epoch'] + 1
        best_f1 = checkpoint['best_f1']
        best_f1_last_10_epoch = checkpoint['best_f1_last_10_epoch']

    if args.pretrained is not None and args.resume is None:
        checkpoint = torch.load(f"{args.pretrained}", map_location='cpu')
        model_without_ddp.load_state_dict(checkpoint['model'])
    ...
    for epoch in range(start_epoch, args.nepochs):
        ...
        with tqdm(total=len(train_loader), unit='img') as pbar:
            for i, data in enumerate(train_loader):
                ...
                if args.instance_seg:
                    loss_ce, loss_coord, loss_seg, loss_instance_seg = loss_dict['loss_ce'], loss_dict['loss_bbox'] * args.coord_loss_coef, loss_dict['loss_seg'], loss_dict['loss_instance_seg']
                    loss = loss_ce + loss_coord + loss_seg + loss_instance_seg
                else:
                    loss_ce, loss_coord, loss_seg = loss_dict['loss_ce'], loss_dict['loss_bbox'] * args.coord_loss_coef, loss_dict['loss_seg']
                ...
                if args.clip_max_norm > 0:
                    torch.nn.utils.clip_grad_norm_(RNGDetNet.parameters(), args.clip_max_norm)
                opt.step()
                        ...
                        for ii in range(3):
                            for jj in range(2):
                                if not (ii==2 and jj==1):
                                    ...
                                    draw.ellipse([delta_x-1,delta_y-1,delta_x+1,delta_y+1],fill='orange') #Current point
                                    if list_len[-1]: 
                                        for kk in range(list_len[-1]):
                                            ...
                                            draw.ellipse([delta_x-1+(v_next[0]*args.ROI_SIZE//2),delta_y-1+(v_next[1]*args.ROI_SIZE//2),\
                                                delta_x+1+(v_next[0]*args.ROI_SIZE//2),delta_y+1+(v_next[1]*args.ROI_SIZE//2)],fill='cyan') #Next GT point
                                    ...
                                    for jj in range(pred_coords.shape[0]):
                                            ...
                                            if pred_probs[jj][0] < pred_probs[jj][1]:
                                                draw.ellipse((v[0]-1,v[1]-1,v[0]+1,v[1]+1),fill='yellow',outline='yellow') #Predicted non-existing
                                            else:
                                                draw.ellipse((v[0]-1,v[1]-1,v[0]+1,v[1]+1),fill='pink',outline='pink') #Prediction
        ...
        if local_rank==0:
            ...
            checkpoint = {
                ...
                'epoch': epoch,
                'best_f1': best_f1,
                'best_f1_last_10_epoch' : best_f1_last_10_epoch,
                'args': args}
            
            print(f'Save checkpoint {epoch}')
            torch.save(checkpoint,os.path.join(args.savedir+'/checkpoints', f"RNGDetNet_{epoch}.pt"))

            if best:
                torch.save(checkpoint,os.path.join(args.savedir+'/checkpoints', f"RNGDetNet_best.pt"))
            if best10:
                torch.save(checkpoint,os.path.join(args.savedir+'/checkpoints', f"RNGDetNet_best_last_10_epoch.pt"))
...
def evaluate(args,RNGDetNet):
    ...
    for name in os.listdir(f'./{args.savedir}/valid/skeleton'):
        ...
        gt_graph = np.array(Image.open(f'{args.dataroot}/segment/{name}'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume',  type=str, default=None,
                         help='Path to the checkpoint from where the training will continue')
    parser.add_argument('--pretrained',  type=str, default=None,
                         help='Path to the checkpoint that is used as a source model for finetuneing')
    ...
    parser.add_argument('--coord_loss_coef', default=5, type=float)
