...
class Agent(FrozenClass):
    ...
    def load_data_and_initialization(self,tile_name):
        ...
        '''
        # load images
        self.sat_image = np.array(Image.open(os.path.join(self.args.dataroot,f'{self.args.data_dir}/region_{tile_name}_sat.png')))
        ...
        try:
            ...
            for i in range(self.image_size//self.crop_size+1):
                for j in range(self.image_size//self.crop_size+1):
                    st_x_ROI = self.crop_size//2+i*self.crop_size
                    nd_x_ROI = self.crop_size//2+(i+1)*self.crop_size

                    st_y_ROI = self.crop_size//2+j*self.crop_size
                    nd_y_ROI = self.crop_size//2+(j+1)*self.crop_size

                    #st_x_ROI = i*self.crop_size
                    #nd_x_ROI = (i+1)*self.crop_size

                    #st_y_ROI = j*self.crop_size
                    #nd_y_ROI = (j+1)*self.crop_size

                    sat_ROI = self.sat_image[st_x_ROI:nd_x_ROI, st_y_ROI:nd_y_ROI]
                    
                    historical_ROI = self.historical_map[st_x_ROI:nd_x_ROI, st_y_ROI:nd_y_ROI]
                    ...
                    pred_mask_text = str(outputs['pred_masks'])

                    with open(f"{self.args.agent_savedir}/segmentation/output_vis/{name}_{i}_{j}.txt", "w") as f:
                        f.write(pred_mask_text)

                    binary_mask[st_x_ROI:nd_x_ROI, st_y_ROI:nd_y_ROI] = pred_mask
                    point_mask[st_x_ROI:nd_x_ROI, st_y_ROI:nd_y_ROI] = pred_point

                    Image.fromarray(pred_mask.astype(np.uint8)).convert('RGB').save(
                        f'./{self.args.agent_savedir}/segmentation/output_vis/{name}_{i}_{j}_pred_mask.png')
                    
                    Image.fromarray(pred_point.astype(np.uint8)).convert('RGB').save(
                        f'./{self.args.agent_savedir}/segmentation/output_vis/{name}_{i}_{j}_pred_point.png')
    ...
    def pop(self):
        ...
        '''
        if len(self.candidate_initial_vertices) == 0:
    ...
    def get_valid_coords(self,pred_logits,pred_coords,thr=0.5):
        ...
        pred_coords_world = [v for v in pred_coords_world if v[0]>=1 and v[1]>=1 and v[0]<self.image_size and v[1]<self.image_size]
