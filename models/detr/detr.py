...
class DETR(nn.Module):
    ...
    def __init__(self, args, segment_backbone, keypoint_backbone, history_backbone, transformer, 
                 num_classes, num_queries, aux_loss=False):
        ...
        if not args.multi_scale:
            ...
            self.input_proj = nn.Conv2d(segment_backbone.num_channels * 2 , hidden_dim, kernel_size=1)
            self.class_embed = nn.Linear(hidden_dim, num_classes + 1)
            self.bbox_embed = MLP(hidden_dim, hidden_dim, 2, 3)
        else:
            self.transformer = transformer
            self.input_proj1 = nn.Conv2d(segment_backbone.num_channels//4 , hidden_dim, kernel_size=1)
            self.input_proj2 = nn.Conv2d(segment_backbone.num_channels//2 , hidden_dim, kernel_size=1)
            self.input_proj3 = nn.Conv2d(segment_backbone.num_channels , hidden_dim, kernel_size=1)
            self.class_embed = nn.Linear(hidden_dim, num_classes + 1)
            self.bbox_embed = MLP(hidden_dim, hidden_dim, 2, 3)
        self.segment_backbone = segment_backbone
        self.keypoint_backbone = keypoint_backbone
    ...
    def forward(self, samples: NestedTensor):
        ...
        features, pos = self.segment_backbone(samples)
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, args):
        ...
        self.bce_loss2 = nn.BCEWithLogitsLoss(weight=torch.Tensor([3]), pos_weight=torch.Tensor([6]))
        self.bce_loss3 = nn.BCEWithLogitsLoss(weight=torch.Tensor([5]), pos_weight=torch.Tensor([10]))
        self.args = args
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        segment_loss = self.bce_loss(pred_segment_mask, target_mask)
        point_loss = self.bce_loss2(pred_point_mask, target_point_mask)
        
        return segment_loss + point_loss
...
def build(args):
    ...
    segment_backbone = build_backbone(args)

    keypoint_backbone = build_backbone(args)
    ...
    if args.frozen_weights is not None:
        model.load_state_dict(torch.load(args.frozen_weights, map_location='cpu')['model'])
        print(model)
