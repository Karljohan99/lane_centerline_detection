...
class DETRsegm(nn.Module):
    ...
    def forward(self, samples: NestedTensor, history_samples: NestedTensor):
        ...
        out = {}
        segment_features, segment_pos = self.detr.segment_backbone(samples)
        keypoint_features, keypoint_pos = self.detr.keypoint_backbone(samples)
        bs = segment_features[-1].tensors.shape[0]

        pred_segment_mask = self.segment_segmentation_head([segment_features[3].tensors,
                                                            segment_features[2].tensors, 
                                                            segment_features[1].tensors, 
                                                            segment_features[0].tensors])
        
        pred_keypoint_mask = self.point_segmentation_head([keypoint_features[3].tensors,
                                                           keypoint_features[2].tensors, 
                                                           keypoint_features[1].tensors, 
                                                           keypoint_features[0].tensors])

        cat_tensor = torch.cat([pred_segment_mask, pred_keypoint_mask],dim=1)
        out["pred_masks"] = cat_tensor
        cat_pred_segment_mask = torch.cat([pred_segment_mask, pred_segment_mask],dim=1)
        segmentation_map = cat_pred_segment_mask.clone().detach().sigmoid()
        ...
        if self.detr.args.instance_seg:
            hs, seg_masks = self.multi_scale_head(segment_features, keypoint_features, features2, self.detr.query_embed.weight, segment_pos)
            outputs_seg_masks = seg_masks.view(bs, self.detr.num_queries, seg_masks.shape[-2], seg_masks.shape[-1])
            out["pred_instance_masks"] = outputs_seg_masks
        else:
            hs = self.multi_scale_head(segment_features, keypoint_features, features2, self.detr.query_embed.weight, segment_pos)
...
class multi_scale(nn.Module):
    ...
    def forward(self, segment_features, keypoint_features, features2, query_embed_weight, pos):
        c2, c3, c4, c5 = segment_features
