...
def graphDensify(node_neighbor, density = 0.000020):
	visited = []
...
def graph2RegionCoordinate(node_neighbor, region, img_size=2048, rot=0):
	transformer = Transformer.from_crs(4326, 3301)
	new_node_neighbor = {}

	for node, nei in node_neighbor.items():
		loc0 = node

		y_loc0, x_loc0 = transformer.transform(loc0[1], loc0[0])

		x0 = int((x_loc0 - region[0])/(region[2]-region[0])*img_size)
		y0 = int((region[3] - y_loc0)/(region[3]-region[1])*img_size)

		for loc1 in nei:
			y_loc1, x_loc1 = transformer.transform(loc1[1], loc1[0])

			x1 = int((x_loc1 - region[0])/(region[2]-region[0])*img_size)
			y1 = int((region[3] - y_loc1)/(region[3]-region[1])*img_size)

			if rot == 0:
				n1key = (y0,x0)
				n2key = (y1,x1)
			else:
				n1key = rotate(x0, y0, rot, img_size//2, img_size//2)
				n2key = rotate(x1, x1, rot, img_size//2, img_size//2)
...
def graphVis(node_neighbor, region, img_size, filename, rot=0):
	transformer = Transformer.from_crs(4326, 3301)
	...
	for node, nei in node_neighbor.items():
		...
		if rot > 0:
			x0, y0 = rotate(x0, y0, rot, img_size//2, img_size//2)
		...
		for loc1 in nei:
			...
			if rot > 0:
				x1, y1 = rotate(x1, y1, rot, img_size//2, img_size//2)
...
def graphVisSegmentation(node_neighbor, region, filename, img_size=2048, rot=0):
	transformer = Transformer.from_crs(4326, 3301)
