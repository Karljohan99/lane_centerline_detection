...
def graph2RegionCoordinate(node_neighbor, region, img_size=2048, rot=0):
	transformer = Transformer.from_crs(4326, 3059)
	new_node_neighbor = {}

	for node, nei in node_neighbor.items():
		loc0 = node

		y_loc0, x_loc0 = transformer.transform(loc0[0], loc0[1])

		x0 = int((x_loc0 - region[1])/(region[3]-region[1])*img_size)
		y0 = int((region[2]-y_loc0)/(region[2]-region[0])*img_size)

		for loc1 in nei:
			y_loc1, x_loc1 = transformer.transform(loc1[0], loc1[1])

			x1 = int((x_loc1 - region[1])/(region[3]-region[1])*img_size)
			y1 = int((region[2]-y_loc1)/(region[2]-region[0])*img_size)
