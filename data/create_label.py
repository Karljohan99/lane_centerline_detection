...
create_directory('vis',delete=True)
create_directory('graph',delete=True)
create_directory('intersection',delete=True)
create_directory('segment',delete=True)
...
for file in os.scandir(DATAROOT):
    ...
    tile_index = "_".join(pieces[1:len(pieces)-3])
    ...
    gt_graph = pickle.load(open(f"{DATAROOT}/region_{tile_index}_refine_gt_graph.p",'rb'))
    graph = Graph()
    ...
    for e in temp_edges:
        ...
        if not whether_inside(src) and not whether_inside(dst):
            if e in src.neighbor_edges:
                src.neighbor_edges.remove(e)
            if e in dst.neighbor_edges:
                dst.neighbor_edges.remove(e)
        else:
    ...
    with open(f'graph/{tile_index}.json','w') as jf:
    ...
    vis_map.save(f'vis/{tile_index}.png')
    ...
    global_mask.save(f'segment/{tile_index}.png')
    ...
    global_keypoint_map.save(f'intersection/{tile_index}.png')
